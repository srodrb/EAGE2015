...
def main(argv):
    ...
    # separamos la diagonal principal de la matriz
    # y la guardamos para almacenarla despues.
    D = M.diagonal()
    
    # guardamos a disco la diagonal
    np.savetxt( fname + ".dia", np.column_stack((D.real.flatten(), D.imag.flatten())) )

    # extramos la diagonal de la matriz
    D = sp.diags( D, 0, shape=(dim,dim) )
    M = M - D
    ...
    np.savetxt( fname + '.val',   ELL_real.flatten()                       )
